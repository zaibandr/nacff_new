...
DynaTreeNode.prototype = {
	...
	_getInnerHtml: function() {
		...
		if ( data.icon === 'false' ) { // icon == false means 'no icon'
			res += "<span style='float:left; width:45px;'>&nbsp;</span>";
			noop(); // keep JSLint happy
		} else if ( data.icon ) {
			var d = data.icon.split(';'); d.sort();
			if (d.length > 1) { 
				res += "<span style='float:left;width:45px'>";
				for(var i in d) res += "<img align='right' src='" + opts.imagePath + d[i] + "' alt='' />"; res += "</span>";
			} else { res += "<span style='float:left;width:45px'><img align='right' style='' src='" + opts.imagePath + data.icon + "' alt='' /></span>"; }
		} else if ( data.icon == "" ) {
			res += "<span style='float:left;width:45px;'>&nbsp;</span>";
		} else { // icon == null means 'default icon'
...
//				this.tree.logDebug("nodeTitle: " + nodeTitle);
		...
		//res += "<div id='additional"+data.id+"' style='margin-left:15px; display:none'>Материал: <input type='text' /></div>";
		return res;
	},
...
//		this.tree.logDebug("%s.render(%s)", this, useEffects);
		...
		if( !parent && !this.ul ) {
			...
			if( ! this.li ) {
				...
				//this.span.setAttribute('style', 'width:auto;');
...
//			window.console.log("%s", node);
	...
	getNodeByID: function(id) {
		// Search the DOM by element ID (assuming this is faster than traversing all nodes).
		// $("#...") has problems, if the key contains '.', so we use getElementById()
		var el = document.getElementById(this.options.idPrefix + id);
		if( el ){
			return el.dtnode ? el.dtnode : null;
		}
		// Not found in the DOM, but still may be in an unrendered part of tree
...
$.ui.dynatree.prototype.options = {
	...
	fx: null, //{ height: "toggle", duration: 200 }, // Animations, e.g. null or { height: "toggle", duration: 200 }
...
$.ui.dynatree.nodedatadefaults = {
	...
	noLink: true, // Use <span> instead of <a> tag for this node
